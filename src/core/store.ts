...
export function validState(value:unknown):value is SavedState {
  ...
  return (v.cultivation===undefined||validCultivation(v.cultivation,v.totalSessions)) && v.version===1 && !!s && !!p && ['focus','break'].includes(s.phase) && ['idle','running','paused','completed'].includes(s.status)
