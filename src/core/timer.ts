import { placeDecoration } from '../shared/living';
...
export class FocusTimer {
  ...
  dispatch(command:Command): void {
    ...
    const s=this.state.session;
    switch(command.type) {
      case 'decoration':placeDecoration(this.state,command.kind,command.to);break;
      case 'seed':
      ...
      case 'preferences': {
        ...
        if(v.gardenStyle!==undefined){
          if(!['simple','living'].includes(v.gardenStyle))throw new Error('Choose Simple or Living garden.');
          next.gardenStyle=v.gardenStyle;
        }
