...
Run the **Windows preview** workflow manually against the version tag being released (for example `v0.4.0`). It builds a portable x64 bundle, runs unit and packaged native tests, and saves a ZIP/checksum artifact for three days. After verifying the run, download `windows-preview` and attach its ZIP and `SHA256SUMS-Windows.txt` to the matching version release. Keep all operating-system downloads under one release per version; do not create separate Windows release entries. Clearly label the Windows asset and release notes as a preview until physical-machine testing is complete. No code-signing is configured for Windows yet.
